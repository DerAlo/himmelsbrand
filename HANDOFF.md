...
- Zwischensequenzen: `showCutscene(key, after)` (liest `CUTSCENES` **und** `BTEAM_CUTSCENES`). Dynamische Cutscenes (`cs.dynamic`, z. B. `ending`/`epilog`) wählen ihren Body über `cs.variants[storyChoice]`; ihre Voice-Over-ID ist `key+'_'+storyChoge`. `cs.next` verkettet automatisch eine weitere Cutscene, bevor der eigentliche `after`-Callback läuft (z. B. `choice` → `ending`). Vor m0 läuft einmalig ein Prolog (`def.cutsceneBefore`, Gate `SAVE.seenProlog`); nach der letzten Mission einer Kampagne zeigt der Debrief-Button „EPILOG ▶" statt „NÄCHSTE MISSION", wenn `def.cutsceneAfter` gesetzt ist.
...
- **Story-Trigger + Barks** (Sektion „15f" direkt vor Sektion 16, nach `updateBoss`): ein einmalig registrierter Dispatcher liest pro Event (`EV.on(...)`) die aktuelle `mission.def.mid[]`-Tabelle und spricht die erste noch nicht gefeuerte, passende Zeile (`on`-Typen: `airborne, time, progress, done, allyHp, hpLow, bossSpawn, bossPart, bossExposed, bossPhase, bossHp, bossKilled, landReady`; `delay` per `setTimeout`, per `missionGen`-Zähler gegen Altlasten aus vorigen Missionen abgesichert). Ohne passenden Eintrag fällt er auf einen kontextabhängigen Bark zurück (`BARK_LANDREADY`/`BARK_DONE`, Cooldown `BARK_COOLDOWN=7s`, kein Sofort-Repeat). **Bosse (BOSSES-Track) müssen dafür `EV.emit('bossSpawn'|'bossPart'|'bossExposed'|'bossPhase'|'bossHp'|'bossKilled', …)` aus `updateBoss`/`checkBossExpose` feuern** (siehe `STORY.md` für die exakten Payload-Felder) — bis dahin bleiben die zugehörigen `mid[]`-Zeilen stumm, alles andere läuft bereits.
